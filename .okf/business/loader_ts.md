...
Modul **loader.ts** digunakan untuk menangani proses bisnis terkait kapabilitas loader.ts di dalam sistem.
...
- Memastikan proses loader.ts berjalan sesuai aturan perusahaan.
- Mengelola transaksi dan validasi data loader.ts.
...
| Aktor | Peran |
|--------|------|
| User / Customer | Memulai transaksi / interaksi loader.ts |
...
- Parameter input loader.ts valid.
- User memiliki kewenangan akses fitur loader.ts.
...
```mermaid
flowchart TD
    A[Mulai Transaksi loader.ts] --> B[Input Data Parameter]
...
Pengguna memasukkan informasi atau payload transaksi loader.ts.
