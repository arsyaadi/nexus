...
Modul **src/planner/index.ts** digunakan untuk menangani proses bisnis terkait kapabilitas src/planner/index.ts di dalam sistem.
...
- Memastikan proses src/planner/index.ts berjalan sesuai aturan perusahaan.
- Mengelola transaksi dan validasi data src/planner/index.ts.
...
| Aktor | Peran |
|--------|------|
| User / Customer | Memulai transaksi / interaksi src/planner/index.ts |
...
- Parameter input src/planner/index.ts valid.
- User memiliki kewenangan akses fitur src/planner/index.ts.
...
```mermaid
flowchart TD
    A[Mulai Transaksi src/planner/index.ts] --> B[Input Data Parameter]
...
Pengguna memasukkan informasi atau payload transaksi src/planner/index.ts.
