...
Modul **package-lock.json** digunakan untuk menangani proses bisnis terkait kapabilitas package-lock.json di dalam sistem.
...
- Memastikan proses package-lock.json berjalan sesuai aturan perusahaan.
- Mengelola transaksi dan validasi data package-lock.json.
...
| Aktor | Peran |
|--------|------|
| User / Customer | Memulai transaksi / interaksi package-lock.json |
...
- Parameter input package-lock.json valid.
- User memiliki kewenangan akses fitur package-lock.json.
...
```mermaid
flowchart TD
    A[Mulai Transaksi package-lock.json] --> B[Input Data Parameter]
...
Pengguna memasukkan informasi atau payload transaksi package-lock.json.
