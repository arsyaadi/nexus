...
Modul **command** digunakan untuk menangani proses bisnis terkait kapabilitas command di dalam sistem.
...
- Memastikan proses command berjalan sesuai aturan perusahaan.
- Mengelola transaksi dan validasi data command.
...
| Aktor | Peran |
|--------|------|
| User / Customer | Memulai transaksi / interaksi command |
...
- Parameter input command valid.
- User memiliki kewenangan akses fitur command.
...
```mermaid
flowchart TD
    A[Mulai Transaksi command] --> B[Input Data Parameter]
...
Pengguna memasukkan informasi atau payload transaksi command.
