...
Modul **OKF (Open Knowledge Format)** digunakan untuk menangani proses bisnis terkait kapabilitas OKF (Open Knowledge Format) di dalam sistem.
...
- Memastikan proses OKF (Open Knowledge Format) berjalan sesuai aturan perusahaan.
- Mengelola transaksi dan validasi data OKF (Open Knowledge Format).
...
| Aktor | Peran |
|--------|------|
| User / Customer | Memulai transaksi / interaksi OKF (Open Knowledge Format) |
...
- Parameter input OKF (Open Knowledge Format) valid.
- User memiliki kewenangan akses fitur OKF (Open Knowledge Format).
...
```mermaid
flowchart TD
    A[Mulai Transaksi OKF (Open Knowledge Format)] --> B[Input Data Parameter]
...
Pengguna memasukkan informasi atau payload transaksi OKF (Open Knowledge Format).
