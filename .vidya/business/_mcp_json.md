...
Modul **.mcp.json** digunakan untuk menangani proses bisnis terkait kapabilitas .mcp.json di dalam sistem.
...
- Memastikan proses .mcp.json berjalan sesuai aturan perusahaan.
- Mengelola transaksi dan validasi data .mcp.json.
...
| Aktor | Peran |
|--------|------|
| User / Customer | Memulai transaksi / interaksi .mcp.json |
...
- Parameter input .mcp.json valid.
- User memiliki kewenangan akses fitur .mcp.json.
...
```mermaid
flowchart TD
    A[Mulai Transaksi .mcp.json] --> B[Input Data Parameter]
...
Pengguna memasukkan informasi atau payload transaksi .mcp.json.
